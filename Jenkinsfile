    ...
    stage 'Stage 2'
        ...
        stage 'Plan'
            withCredentials([string(credentialsId: 'c8178bdb-a72f-4ede-91d3-190d6a11af15', variable: 'ACCESS_KEY'), string(credentialsId: 'db568c29-c2e0-407b-a93b-463f604a3553', variable: 'SECRET_KEY')]) {
                ...
                sh "terraform remote config -backend=s3 -backend-config='bucket=us-west-2a.terraform.remotestate.sb' -backend-config='key=JenkinsPlayground/terraform.tfstate' -backend-config='us-west-2'"
